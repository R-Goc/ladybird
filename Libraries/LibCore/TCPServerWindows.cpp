...
ErrorOr<NonnullRefPtr<TCPServer>> TCPServer::try_create()
{
    ...
    ArmedScopeGuard close_handle { [&handle]() {
        MUST(Core::System::close(handle.release()));
    } };

    int option = 1;
    TRY(Core::System::ioctl(handle, FIONBIO, &option));
    TRY(Core::System::setsockopt(handle, SOL_SOCKET, SO_OOBINLINE, &option, sizeof(option)));
    // FIXME: Like in LocalServerWindows, verify that this is correct
    if (SetHandleInformation(reinterpret_cast<HANDLE>(handle.socket()), HANDLE_FLAG_INHERIT, 0) == 0)
...
ErrorOr<void> TCPServer::listen(IPv4Address const& address, u16 port, AllowAddressReuse allow_address_reuse)
{
    ...
    if (allow_address_reuse == AllowAddressReuse::Yes) {
        int option = 1;
        TRY(Core::System::setsockopt(m_handle, SOL_SOCKET, SO_REUSEADDR, &option, sizeof(option)));
    }

    TRY(Core::System::bind(m_handle, (sockaddr const*)&in, sizeof(in)));
    TRY(Core::System::listen(m_handle, 5));
    m_listening = true;
...
ErrorOr<void> TCPServer::set_blocking(bool const blocking)
{
    ...
    int option = 1;
    TRY(Core::System::ioctl(m_handle, FIONBIO, &option));
    return {};
}
...
ErrorOr<NonnullOwnPtr<TCPSocket>> TCPServer::accept()
{
    ...
    int accepted_fd = TRY(Core::System::accept(m_handle.socket(), (sockaddr*)&in, &in_size));
    return TRY(TCPSocket::adopt_handle(PlatformHandle { NativeSocketType(accepted_fd) }));
}
