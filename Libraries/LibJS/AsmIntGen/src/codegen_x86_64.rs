...
impl AbiContext {
    fn new(fmt: ObjectFormat) -> Self {
        let m = mapping_for(Arch::X86_64, fmt);
        let arg0 = m.arguments[0].to_string();
        let arg1 = m.arguments[1].to_string();
        let arg1_32 = to_32bit_reg(&arg1);
        let arg2 = m.arguments[2].to_string();
        let arg3 = m.arguments[3].to_string();
        let interp_offset = if fmt == ObjectFormat::COFF { 64 } else { 48 };
        Self {
            arg0,
            arg1_32,
            arg2,
...
pub fn generate(program: &Program) -> String {
    ...
    let abi = AbiContext::new(program.object_format);
...
fn emit_cet_endbr(out: &mut String) {
    w!(out, "    endbr64");
}
...
fn emit_file_trailer(out: &mut String, fmt: ObjectFormat) {
    if fmt == ObjectFormat::COFF {
        // FIXME: Not supported in lld-link
        // Signal CFG and CET compatibility to the Windows linker
        //  w!(out, ".section .drectve,\"yn\"");
        // w!(out, "    .ascii \" /CETCOMPAT\"");
        w!(out);
        w!(out, ".section .sxdata,\"dr\"");
        w!(out, "    .globl @feat.00");
        w!(out, "@feat.00 = 0x80000101 # CFG + CET + GS");
    } else if fmt != ObjectFormat::MachO {
...
fn generate_entry_point(out: &mut String, program: &Program, abi: &AbiContext) {
    ...
    if fmt == ObjectFormat::COFF {
        // Shadow space (32 bytes) + alignment (8 bytes) = 40 bytes
        w!(out, "    sub rsp, 40");
    ...
    let arg0 = &abi.arg0;
    let arg1_32 = &abi.arg1_32;
    let arg2 = &abi.arg2;
    let arg3 = &abi.arg3;
    let interp_offset = abi.interp_offset;
    w!(out, "    mov r14, {arg0}          # pb = bytecode base");
    w!(out, "    mov r13d, {arg1_32}         # pc = entry_point");
    w!(out, "    mov r15, {arg2}           # values = values array");
    // Store VM* on the stack for C++ calls, pin exec_ctx instead
    w!(
        out,
        "    mov QWORD PTR [rbp - {interp_offset}], {arg3}   # save VM*"
    );
...
fn emit_call_wrapper(out: &mut String, func: &str) {
    w!(out, "    call CSYM({func})");
}

fn generate_fallback_handler(out: &mut String, program: &Program, abi: &AbiContext) {
    ...
    // Set up args: arg0=vm (from stack), arg1=pc (r13d)
    let arg0 = &abi.arg0;
    let interp_offset = abi.interp_offset;
    let arg1_32 = &abi.arg1_32;
    w!(out, "    mov {arg0}, QWORD PTR [rbp - {interp_offset}]");
    w!(out, "    mov {arg1_32}, r13d");

    emit_call_wrapper(out, "asm_fallback_handler");
...
/// from the VM* saved on the stack. Uses abi.arg3 as scratch.
fn emit_state_reload(out: &mut String, program: &Program, abi: &AbiContext) {
    let interp_ctx = program
        .constants
        .get("VM_RUNNING_EXECUTION_CONTEXT")
        .copied()
    ...
    let arg3 = &abi.arg3;
    let interp_offset = abi.interp_offset;
    w!(out, "    mov {arg3}, QWORD PTR [rbp - {interp_offset}]");
    w!(out, "    mov rbx, QWORD PTR [{arg3} + {interp_ctx}]");
    w!(out, "    mov {arg3}, QWORD PTR [rbx + {exec_executable}]");
    w!(out, "    mov r14, QWORD PTR [{arg3} + {exec_bytecode}]");
...
fn handler_size(handler: &Handler, program: &Program) -> u32 {
    ...
    let layout = program.op_layouts.get(&handler.name).unwrap_or_else(|| {
        panic!(
            "No Bytecode.def layout for handler '{name}' and no explicit size",
            name = handler.name
        )
    });
    layout.size.unwrap_or_else(|| {
        panic!(
            "Handler '{name}' is variable-length; use dispatch_variable instead of bare dispatch",
            name = handler.name
        )
    }) as u32
}

fn generate_handler(out: &mut String, handler: &Handler, program: &Program, abi: &AbiContext) {
...
fn resolve_op(op: &Operand, handler: &Handler, program: &Program) -> String {
    match op {
        Operand::Register(name) => resolve_register(name, Arch::X86_64, program.object_format)
        ...
        Operand::Memory { base, index, scale } => {
            let base_r = resolve_register(base, Arch::X86_64, program.object_format)
                .unwrap_or_else(|| base.clone());
            match (index, scale) {
                (Some(idx), Some(sc)) => {
                    let idx_r = resolve_register(idx, Arch::X86_64, program.object_format)
    ...
    match m.as_str() {
        ...
        "call_slow_path" => {
            if let Some(Operand::Register(func_name)) = insn.operands.first() {
                // Set up args: arg0=vm (from stack), arg1=pc
                let arg0 = &abi.arg0;
                let interp_offset = abi.interp_offset;
                let arg1_32 = &abi.arg1_32;
                w!(out, "    mov {arg0}, QWORD PTR [rbp - {interp_offset}]");
                w!(out, "    mov {arg1_32}, r13d");
                emit_call_wrapper(out, func_name);
                // Check for exit (return < 0)
                w!(out, "    test rax, rax");
                w!(out, "    js .Lexit");
        ...
        // The value from t1 is passed as the first argument.
        // Result is in rax (t0). NOT terminal: handler continues after.
        "call_helper" => {
            if let Some(Operand::Register(func_name)) = insn.operands.first() {
                let t1 = resolve_register("t1", Arch::X86_64, program.object_format).unwrap();
                let arg0 = &abi.arg0;
                w!(out, "    mov {arg0}, {t1}");
                emit_call_wrapper(out, func_name);
            }
        ...
        "double_to_int32" => {
            if insn.operands.len() >= 3 {
                ...
                let t1 = crate::registers::resolve_register(
                    "t1",
                    crate::registers::Arch::X86_64,
                    program.object_format,
                )
                .unwrap();
                let ft3 = crate::registers::resolve_register(
                    "ft3",
                    crate::registers::Arch::X86_64,
                    program.object_format,
                )
                ...
                w!(out, "    mov {t1}, 0x8000000000000000");
                w!(out, "    cmp {dst}, {t1}");
                w!(out, "    je {fail}");
                // Check it fits in i32
                w!(out, "    movsxd {t1}, {dst32}");
                w!(out, "    cmp {dst}, {t1}");
                w!(out, "    jne {fail}");
                // Round-trip: convert back and compare
                w!(out, "    cvtsi2sd {ft3}, {dst}");
                w!(out, "    ucomisd {src}, {ft3}");
                w!(out, "    jne {fail}");
            }
        }
        ...
        "fp_add" | "fp_sub" | "fp_mul" | "fp_div" | "fp_sqrt" => {
            ...
            let ops: Vec<String> = insn
                ...
                .collect();
            let joined_ops = ops.join(", ");
            w!(out, "    {x86_mnemonic} {joined_ops}");
        }
        ...
        "canonicalize_nan" => {
            if insn.operands.len() == 2 {
                ...
                let id = state.unique_counter;
                state.unique_counter += 1;
                let fixup_label = format!(".Lasm_{handler_name}.canon_nan_{id}");
                let ret_label = format!(".Lasm_{handler_name}.canon_nan_{id}_ret");
                w!(out, "    movq {dst}, {src}");
        ...
        "jmp" => {
            if let Some(Operand::Label(label)) = insn.operands.first() {
                if label.starts_with('.') {
                    w!(out, "    {m} .Lasm_{handler_name}{label}");
                } else {
                    w!(out, "    {m} {label}");
                }
...
mod tests {
    ...
    fn lowers_high_single_bit_branch_mask_with_bt() {
        ...
        program.constants.insert("HIGH_BIT".into(), 0x1_0000_0000);
        let handler = call_handler();
