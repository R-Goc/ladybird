...
ErrorOr<Process::ProcessAndIPCTransport> Process::spawn_and_connect_to_process(Core::ProcessSpawnOptions const& options)
{
    ...
    Core::PlatformHandle socket_fds[2] {};
    TRY(Core::System::socketpair(AF_LOCAL, SOCK_STREAM, 0, socket_fds));

    ArmedScopeGuard guard_fd_0 { [&] { MUST(Core::System::close(move(socket_fds[0]))); } };
    ArmedScopeGuard guard_fd_1 { [&] { MUST(Core::System::close(move(socket_fds[1]))); } };
    ...
    auto takeover_string = MUST(String::formatted("{}:{}", options.name, socket_fds[1].as_socket()));
    ...
    auto ipc_socket = TRY(Core::LocalSocket::adopt_handle(socket_fds[0]));
    guard_fd_0.disarm();
...
ErrorOr<OwningPlatformHandle> Process::create_ipc_socket(ByteString const& socket_path)
{
    if (!Core::System::stat(socket_path).is_error())
        TRY(Core::System::unlink(socket_path));

#    ifdef SOCK_NONBLOCK
    auto socket_handle = TRY(Core::System::socket(AF_LOCAL, SOCK_STREAM | SOCK_NONBLOCK | SOCK_CLOEXEC, 0));
#    else
    auto socket_handle = TRY(Core::System::socket(AF_LOCAL, SOCK_STREAM, 0));

    int option = 1;
    TRY(Core::System::ioctl(socket_handle.as_socket(), FIONBIO, &option));
    TRY(Core::System::fcntl(socket_handle.as_socket(), F_SETFD, FD_CLOEXEC));
#    endif

#    if !defined(AK_OS_BSD_GENERIC) && !defined(AK_OS_GNU_HURD)
    TRY(Core::System::fchmod(socket_handle.as_socket(), 0600));
#    endif
    ...
    TRY(Core::System::bind(socket_handle.as_socket(), reinterpret_cast<sockaddr*>(&socket_address_un), sizeof(socket_address_un)));
    TRY(Core::System::listen(socket_handle.as_socket(), 16));
